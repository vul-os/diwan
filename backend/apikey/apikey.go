...
// public /v1 API to authenticate `Authorization: Bearer vk_…` credentials.
//
// Introspection is brokered by an optional gateway the owner configures ("CP"). Office (and
...
//
// Results are cached in-process for cacheTTL (~60s) so a burst of API calls does
// not hammer the CP and a single key is introspected at most once per minute.
//
...
const (
	...
	// ProductOffice is the product scope a key MUST carry to use the office /v1
	...
	// When unset the key path is disabled and office falls back to session-only
	// auth (self-host unchanged). This is the SAME env the cloud billing seam uses.
	EnvCPBaseURL = "VULOS_CP_BASE_URL"

	// EnvCPToken is the service token office presents to the CP on the
