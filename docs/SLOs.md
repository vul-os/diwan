# Diwan – Service Level Objectives
...
| 2 | **File API p95** | < 300 ms (GET /api/files/:id) | `diwan_request_duration_seconds` p95 | 43.2 min/month | p95 > 600 ms for 5 min → alert |
| 3 | **Signing workflow p99** | < 2 s from POST /sign/:envelope/send | Trace span `signing.send` | 43.2 min/month | p99 > 5 s → alert |
| 4 | **API error rate** | < 0.5% | `diwan_error_count_total / diwan_request_count_total` | 26 min/month | Rate > 2% for 2 min → halt deploy + alert |
