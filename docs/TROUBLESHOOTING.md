...
Collaboration is **peer-to-peer — there is no central document server** to check. Edits travel between browsers over an E2E-encrypted WebRTC room; the only server piece is content-blind peer **discovery**, served by default by this binary itself at `/api/rendezvous/*` (or by a host box's `/api/peering/*`, or by a configured relay). In the Network tab you should see that discovery traffic and a WebRTC connection — and **never** a `/v1/documents/*/collab/*` request (there is none).

| Symptom | Cause | Fix |
|---------|-------|-----|
| Editor shows "Offline"; nobody else appears | This deployment offers **no discovery at all**: the built-in surface is disabled (`collab.builtin_rendezvous: false`), there is no host-box `/api/peering/*`, and no `rendezvous_url` is configured | Check `GET /api/reachability` — if both `builtin_rendezvous_prefix` and `rendezvous_url` are blank, that is the cause. Remove `builtin_rendezvous: false` (the default is on), **or** run Diwan behind a Vulos OS / Ephor host, **or** set `collab.rendezvous_url` to a self-hosted `vulos-relayd` (see [CONFIGURATION.md](CONFIGURATION.md)). Your edits still autosave to your storage regardless |
| Editor shows "Offline" but `/api/reachability` reports `builtin_rendezvous_prefix: "/api/rendezvous"` | A reverse proxy in front of Diwan is not passing `/api/rendezvous/*` through, or is rate-limiting it | `curl -i https://office.example.org/api/rendezvous/healthz` must return `{"ok":true,…}`. It is ordinary same-origin JSON over POST/GET — no WebSocket upgrade needed — so a 404/502 here is the proxy, not Diwan |
| Discovery requests to `/api/rendezvous/*` return **409** | The browser's clock is more than 5 minutes from the server's, so every signed envelope looks stale (this is the replay defence doing its job) | Fix clock sync on either end (NTP). The response body says so explicitly |
| Discovery requests to `/api/rendezvous/*` return **429** | Per-IP rate limit — normally only reachable with many peers behind one NAT, or a client stuck in a reconnect loop | Check for a page reload loop first. A large office behind one address is well inside the limit (240-token burst, 40/s sustained) |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Presence pill says offline/local; peer count stays 0 | No discovery transport is reachable at all | `GET /api/reachability`: if `builtin_rendezvous_prefix` is `/api/rendezvous`, the surface is on — check your proxy passes it through. If it is blank, someone set `collab.builtin_rendezvous: false`; the default is on. A 404 on `/api/peering/stream` is normal and expected on a standalone binary (that path comes from a Vulos OS / Ephor host) |
...
1. **WebSocket to `/api/peering/stream` connected?** Host-box peer discovery is up. A `404` there is normal on a standalone binary — check `GET /api/reachability` instead: `rendezvous_url` non-blank means discovery goes straight to that self-hosted relayd; `builtin_rendezvous_prefix` non-blank (the default) means this binary is serving discovery itself at that path. Only if **both** are blank is collaboration local-only.
