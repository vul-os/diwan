...
//
// Moved out of package main (originally repo-root docs_mirror_test.go): this
...
// docsMirror maps a source file to its site/docs/ copy, both given relative to
// the module root. Extend it when a document is added to the published site —
// the coverage check below will tell you when that is needed.
...
func TestSiteDocsMirrorIsInSync(t *testing.T) {
	...
	for src, dst := range docsMirror {
		srcAbs, dstAbs := repoPath(src), repoPath(dst)
	...
	siteDocsDir := repoPath("site/docs")
	entries, err := os.ReadDir(siteDocsDir)
	if err != nil {
		t.Fatalf("read %s: %v — the orphan check verified NOTHING", siteDocsDir, err)
	}
	md := 0
...
func TestSiteDocsPageListIsMirrored(t *testing.T) {
	page, err := os.ReadFile(repoPath("site/docs.html"))
	if err != nil {
