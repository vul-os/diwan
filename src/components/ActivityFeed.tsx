...
const KIND_CONFIG: Record<string, { icon: LucideIcon; iconCn: string; bgCn: string }> = {
...
function ActivityList({ fileId }: { fileId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await api.getActivity(fileId) as ActivityEvent[]
  ...
  type Row = { type: 'separator'; day: string } | { type: 'event'; ev: ActivityEvent }
  const rows: Row[] = []
...
function SnapshotsTab({ fileId, onRestore }: SnapshotsTabProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast(e instanceof Error ? e.message : 'Failed to create snapshot', false)
    } finally {
      setCreating(false)
    }
  }
...
function VersionRow({ v, idx, restoring, onRestoreClick, isNamed = false }: VersionRowProps) {
  return (
...
export default function ActivityFeed({ fileId, onRestore, onClose }: ActivityFeedProps) {
  const [tab, setTab] = useState<'activity' | 'snapshots'>('activity')
  ...
  return (
    <div className="w-72 flex flex-col border-l border-line bg-paper h-full overflow-hidden animate-slide-in-right">
      ...
      <Tabs
        value={tab}
        onChange={(v) => setTab(v as 'activity' | 'snapshots')}
        items={PANEL_TABS}
