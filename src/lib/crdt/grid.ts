...
export type GridOpKind = typeof GRID_OP_SET | typeof GRID_OP_CLEAR
export type GridCellKey = { r: number; c: number }
export type GridOp = { kind: GridOpKind; id: string; key: GridCellKey; v?: string }
export type GridCellRecord = { r: number; c: number; opId: string; value: string; deleted: boolean }
export type GridCellSnapshot = { r: number; c: number; v: string }

// ---------------------------------------------------------------------------
// GridCRDT — in-memory LWW map keyed by "r,c"
// ---------------------------------------------------------------------------

type GridCellEntry = { opId: string; value: string; deleted: boolean }

class GridCRDT {
  // key: "r,c" → { opId, value, deleted }
  private _cells: Map<string, GridCellEntry>

  constructor() {
    this._cells = new Map()
  }

  apply(op: GridOp): boolean {
  ...
  restore(cells: GridCellRecord[]): void {
    this._cells.clear()
...
const SNAPSHOT_KEY = (id: string) => `crdt_grid_${id}`
const OP_LOG_KEY   = (id: string) => `crdt_grid_ops_${id}`
const MAX_OPLOG    = 500   // cap persisted log entries

export type GridSessionOptions = {
  /** file / document id */
  sessionId: string
  /** stable per-tab id (use sessionStorage) */
  replicaId: string
  /** OFFICE-20 transport; null = local-only */
  fabricClient?: FabricClient | null
}

export type ChartDescriptor = { id: string; [key: string]: unknown }
...
export class GridSession extends EventTarget {
  ...
  private _crdt: GridCRDT
  private _destroyed: boolean
  private _engineGuard: GridEngineGuard
  private _onFabricMessage?: (ev: Event) => void

  constructor({ sessionId, replicaId, fabricClient = null }: GridSessionOptions) {
    super()
    ...
    this._engineGuard = new GridEngineGuard(ENGINE_LOCAL_LWW, (info: EngineMismatch) => {
  ...
  setCell(row: number, col: number, value: unknown): void {
    ...
    const op: GridOp = { kind: GRID_OP_SET, id, key: { r: row, c: col }, v: String(value) }
    this._crdt.apply(op)
  ...
  clearCell(row: number, col: number): void {
    const id = this._clock.tick()
    const op: GridOp = { kind: GRID_OP_CLEAR, id, key: { r: row, c: col } }
    this._crdt.apply(op)
    this._broadcast({ type: 'grid_op', session: this._session, op })
    this._persistOp(op)
    this._emitLocalOp(op)
  ...
  private _emitLocalOp(op: GridOp): void {
  ...
  logSnapshotData(): GridCellRecord[] {
   ...
   */
  cells(): GridCellSnapshot[] { return this._crdt.cells() }
  ...
  private _handleFabricMessage(raw: unknown): void {
    ...
    if (msg.type === 'grid_op' && msg.op) {
      ...
      this._ingestSnapshotCells(msg.cells as GridCellRecord[])
    }
  }
  ...
  private _ingestGridOp(op: unknown): boolean {
    if (!op) return false
    const gridOp = op as GridOp
    // Advance clock past remote counter.
    if (gridOp.id) {
      const parts = String(gridOp.id).split('_')
      this._clock.observe(parseInt(parts[1], 10) || 0)
    }
    const changed = this._crdt.apply(gridOp)
    if (changed) {
      this._persistOp(gridOp)
      this.dispatchEvent(new CustomEvent('remoteOp', { detail: { op: gridOp } }))
    }
    return changed
  }

  /** Merge a snapshot's cells (cold-join / durable-log hydrate). */
  private _ingestSnapshotCells(cells: GridCellRecord[]): void {
    for (const cell of cells) {
      if (cell.opId) {
        ...
        this._clock.observe(parseInt(parts[1], 10) || 0)
        const kind: GridOpKind = cell.deleted ? GRID_OP_CLEAR : GRID_OP_SET
...
export function getGridReplicaId(): string {
