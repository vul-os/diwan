...
/** Anchor shape: `{ type, from, to, orphaned }` (Docs text-range anchors; other
 *  products may carry additional fields, so this stays open). */
...
export type CommentWithReplies = Comment & { replies: Reply[] }
...
export type CommentOp =
  | { op: typeof OP_ADD_COMMENT | typeof OP_EDIT_COMMENT | typeof OP_RESOLVE_COMMENT | typeof OP_REOPEN_COMMENT
      comment: Comment; applied_at: string }
  | { op: typeof OP_ADD_REPLY | typeof OP_EDIT_REPLY | typeof OP_DELETE_REPLY
      reply: Reply; applied_at: string }

export type CommentStoreOptions = {
  /** broadcast hook */
  onOp?: ((op: CommentOp) => void) | null
}
...
export class CommentStore {
  ...
  private _onOp: ((op: CommentOp) => void) | null
  private _comments: Map<string, Comment>
  private _replies: Map<string, Map<string, Reply>>
  private _ops: CommentOp[]
  private _opKeys: Set<string>

  /**
   * @param nodeId  unique id for this replica
   */
  constructor(nodeId: string, opts: CommentStoreOptions = {}) {
   ...
   */
  loadFromServer(items: (Comment & { replies?: Reply[] })[]): void {
  ...
  addComment(anchor: CommentAnchor, authorId: string, body: string): Comment {
    ...
    const op: CommentOp = { op: OP_ADD_COMMENT, comment, applied_at: now }
    this._applyLocal(op)
    return comment
  }

  editComment(commentId: string, newBody: string): Comment {
    const existing = this._comments.get(commentId)
    if (!existing) throw new Error(`comment not found: ${commentId}`)
    const now = new Date().toISOString()
    const updated: Comment = { ...existing, body: newBody, seq_clock: this._clock.tick(), updated_at: now }
    const op: CommentOp = { op: OP_EDIT_COMMENT, comment: updated, applied_at: now }
    this._applyLocal(op)
    return updated
  }

  resolve(commentId: string): Comment {
    return this._setState(commentId, 'resolved', OP_RESOLVE_COMMENT)
  }

  reopen(commentId: string): Comment {
  ...
  private _setState(
    commentId: string,
    state: CommentState,
    opType: typeof OP_RESOLVE_COMMENT | typeof OP_REOPEN_COMMENT,
  ): Comment {
    const existing = this._comments.get(commentId)
    if (!existing) throw new Error(`comment not found: ${commentId}`)
    const now = new Date().toISOString()
    const updated: Comment = { ...existing, state, seq_clock: this._clock.tick(), updated_at: now }
    const op: CommentOp = { op: opType, comment: updated, applied_at: now }
    this._applyLocal(op)
    return updated
  }

  addReply(commentId: string, authorId: string, body: string): Reply {
    ...
    const op: CommentOp = { op: OP_ADD_REPLY, reply, applied_at: now }
    this._applyLocal(op)
    return reply
  }

  editReply(commentId: string, replyId: string, newBody: string): Reply {
    ...
    const now = new Date().toISOString()
    const updated: Reply = { ...existing, body: newBody, seq_clock: this._clock.tick(), updated_at: now }
    const op: CommentOp = { op: OP_EDIT_REPLY, reply: updated, applied_at: now }
    this._applyLocal(op)
    return updated
  }
  ...
  deleteReply(commentId: string, replyId: string): void {
    ...
    if (!existing) throw new Error(`reply not found: ${replyId}`)
    const now = new Date().toISOString()
    const tombed: Reply = { ...existing, body: '', deleted: true, seq_clock: this._clock.tick(), updated_at: now }
    const op: CommentOp = { op: OP_DELETE_REPLY, reply: tombed, applied_at: now }
    this._applyLocal(op)
  }
  ...
  mergeOps(ops: CommentOp[]): void {
    for (const op of ops) {
      const clock = 'comment' in op ? op.comment?.seq_clock : op.reply?.seq_clock
   ...
   */
  remapAnchors(newMap: Map<string, { from: number; to: number } | null>): void {
   ...
   * @param filterState  'open' | 'resolved' | undefined = all
   */
  list(filterState?: CommentState): CommentWithReplies[] {
  ...
  private _listReplies(commentId: string): Reply[] {
    const rm = this._replies.get(commentId)
    if (!rm) return []
  ...
  exportOps(afterClock = ''): CommentOp[] {
  ...
  private _applyLocal(op: CommentOp): void {
    this._applyToIndex(op)
    this._appendToLog(op)
    if (this._onOp) this._onOp(op)
  }

  private _applyToIndex(op: CommentOp): void {
    switch (op.op) {
      case OP_ADD_COMMENT: {
      ...
      default:
        console.warn('[CommentStore] unknown op:', (op as CommentOp).op)
    }
  }

  private _appendToLog(op: CommentOp): void {
    const clock = ('comment' in op ? op.comment?.seq_clock : op.reply?.seq_clock) || ''
    const id = ('comment' in op ? op.comment?.id : op.reply?.id) || ''
...
const _stores = new Map<string, CommentStore>()

export function getCommentStore(fileId: string, opts: CommentStoreOptions = {}): CommentStore {
...
export function evictCommentStore(fileId: string): void {
